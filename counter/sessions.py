...
class KeyValueSessionStore(SessionStore):
    ...
    # This was my version. Messed with it not sure if it still works
    # def set_key(self, key, value):
    #     r = requests.get(self.url)
    #     obj = {key:value}
    #     requests.post(self.url,data=obj)
    #     # self.logger.debug(requests.post(self.url,data=obj))

    # def get_key(self, key):
    #     r = requests.get(self.url + '/' + key)
    #     return r.json()[key]
    #     # self.logger.debug(requests.get(self.url, params=key))

    # def delete_key(self, key):
    #     requests.delete(url + '/' + key)
        # self.logger.debug(requests.delete(str(self.url) + '/' + str(key)))
    ...
    def set_key(self,key,value):
        ...
        r = requests.get(url)
        test = requests.post(url,json={key: value})
    ...
    def delete_key(self,key):
        url = 'http://127.0.0.1:5100/'
        temporary = requests.delete(url+key)
